...
class Game:
    ...
    def new(self):
        ...
        self.background = pygame.image.load( os.path.join(self.dir_images, 'go_w.jpg') )
    ...
    def draw_text(self):
        ...
        if not self.playing:
            self.display_text( "You Lose :(", 60, BLACK, WIDTH // 2, HEIGHT // 2)
            self.display_text( "Press \"R\" to restart", 30, BLACK, WIDTH // 2, 50)
    ...
    def menu(self):
        self.surface.fill(BLUE_LIGHT)
        self.display_text("Press a button to start", 36, BLACK, WIDTH // 2, 10)
